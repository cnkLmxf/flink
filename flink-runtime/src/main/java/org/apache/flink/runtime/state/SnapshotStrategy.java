...
public interface SnapshotStrategy<S extends StateObject, SR extends SnapshotResources> {
     ...
     * 将快照写入由给定 {@link CheckpointStreamFactory} 提供的流
     * 并返回 @{@link SupplierWithException} 的操作，该流提供快照的状态句柄。
     *
     * @param checkpointId The ID of the checkpoint.
     ...
     * {@link SnapshotResult} 的供应商，可以访问 {@link CloseableRegistry}，
     * 用于在取消检查点的异步部分时需要关闭的 io 任务。
     *
     * @param <S> type of the returned state object that represents the result of the snapshot *
     *     operation.
     */
